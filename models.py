    ...
    async def chat_stream_generator(self, prompt: str, model: str = None, system_prompt: str = None, history: List[Dict[str, str]] = None, progress_callback=None, tool_settings: Dict[str, bool] = None):
        ...
        if tool_settings.get("automator", True):
            automator_instruction = (
                ...
                "REGLAS CRÍTICAS:\n"
                "1. FEEDBACK INICIAL Y SILENCIO DE CÓDIGO: Antes del bloque <run_python>, escribe SIEMPRE un brevísimo mensaje de cortesía indicando al usuario qué acción vas a realizar y pidiéndole que espere unos segundos (ej: 'Entendido. Voy a crear la landing page en tu escritorio, aguarda unos segundos por favor...' o 'Entendido, procedo a limpiar los archivos temporales...'). Después de este mensaje introductorio, coloca únicamente el bloque <run_python>. NO expliques ni comentes el código en sí.\n"
                ...
                "4. ESCRITORIO Y RUTAS DE USUARIO: Para referirte al Escritorio del usuario, NUNCA uses rutas absolutas estáticas de tu entorno. Usa SIEMPRE `import os; desktop = os.path.join(os.path.expanduser('~'), 'Desktop')` (o en su defecto si no existe 'Desktop', usa 'Escritorio'). Asegúrate de escribir los archivos directamente en esa ruta si el usuario pide crearlos o guardarlos en su escritorio.\n"
                ...
                "9. EVITA F-STRINGS CON LLAVES: Si vas a generar o escribir código HTML, CSS, JavaScript o cualquier texto que contenga llaves ({ y }), ¡NO uses f-strings (f'...' o f\"\"\"...\") en Python! Usa cadenas normales (con triple comilla como ''' o \"\"\") o cadenas crudas (r'''...'''). De lo contrario, Python intentará evaluar el contenido de las llaves como código Python y fallará con SyntaxError o KeyError.\n\n"
